...
let inStr16 = `departure location: 33-679 or 691-971
...
var getDay16Input = () => {
    const parts = inStr16.split('\n\n').map(part => part.split('\n'));
    parts[0] = parts[0].map(line => {
        const secs = line.split(":").map(s=>s.trim());
        const ranges = secs[1].split(' or ').map(r => { 
            let limits = r.trim().split('-').map(s => parseInt(s)); 
            return [limits[0], limits[1]]
        });

        return {"name": secs[0], "ranges": ranges} 
    });

    parts[1] = parseTicket(parts[1][1]);

    parts[2] = parts[2].filter((el, i) => i>0).map(s => parseTicket(s));
...
var getDay17Input = (part) => {
    const lines = inStr17.split('\n');

    let yLines = {};

    lines.map(l => {
        let list = {};
        l.split('').forEach((s, i) =>list[i] = s);
        return list;
    }).forEach((v,x) => yLines[x] = v);

    return part == 2? {0:{0: yLines}, 'z':1, 'y': Object.keys(yLines).length+1, 'x':Object.keys(yLines[0]).length+1, 'w':1} :
                        {0: yLines, 'z':1, 'y': Object.keys(yLines).length+1, 'x':Object.keys(yLines[0]).length+1};
}

var getDay18Input = (part) => {
    return part == 1 ?  inStr18.split('\n').map(line => line.split('').filter(c => c != " ")) : 
                        inStr18.split('\n').map(s => s.replace(/ /g, ""));
}

var getDay19Input = (part) => {
    let rules = {};
    let sections = [];

    if(part == 1){
        sections = inStr19.split('\n\n');
    } else {
        sections = inStr19_2.split('\n\n');
    }

    let rules_lines = sections[0];
    ...
    rules_lines.split('\n').map(line => {
        ...
        parts[1] = parts[1].split('|')
                            .map(s => s.trim())
                            .map(s => s.replace(/"[a-z]"/, x => x.substr(1,1)))
                            .map(s => s.replace(/[0-9]+/g, x => "(" + x + ")"))
                            .map(s => s.replace(/ /g, x => ""))
                            .join(")|(");
    ...
    let messages = sections[1].split("\n");

    return [rules, messages];
}

// console.log(getDay19Input());
// console.log("===============");
...
exports.getDay16Input = getDay16Input;
exports.getDay17Input = getDay17Input;
exports.getDay18Input = getDay18Input;
exports.getDay19Input = getDay19Input;